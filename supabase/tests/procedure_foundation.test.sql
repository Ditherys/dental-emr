...
select extensions.is((select count(*)::integer from information_schema.columns where table_schema='public' and table_name in ('procedures','procedure_specialties','procedure_eligible_providers') and column_name ~* 'price|cost|fee|amount' and not (table_name='procedures' and column_name='default_fee_centavos')),'0'::integer,'P3-05 tables contain no price-like columns other than the approved B2 default fee');
