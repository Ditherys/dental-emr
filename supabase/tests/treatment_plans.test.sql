...
select extensions.columns_are('public','treatment_plan_items',array['id','organization_id','plan_id','line_no','procedure_id','tooth_code','description','estimated_fee','created_at','estimated_fee_centavos'],'treatment_plan_items has only the approved P16-01 fields plus the B2 centavo compatibility column');
...
select extensions.lives_ok($$insert into public.treatment_plans (id,organization_id,patient_id,title,status) values ('d1600000-0000-0000-0000-000000000002','b7200000-0000-0000-0000-000000000001','b7500000-0000-0000-0000-000000000001','Presented plan','PRESENTED')$$,'a PRESENTED plan row is accepted directly');
select extensions.throws_ok($$update public.treatment_plans set title='rewrite' where id='d1600000-0000-0000-0000-000000000002'$$,'23514','presented/acknowledged treatment plans are immutable; create a new version','a PRESENTED plan rejects UPDATE by the immutable trigger');
select extensions.throws_ok($$delete from public.treatment_plans where id='d1600000-0000-0000-0000-000000000002'$$,'23514','presented/acknowledged treatment plans are immutable; create a new version','a PRESENTED plan rejects DELETE by the immutable trigger');
...
select extensions.lives_ok($$insert into public.treatment_plan_items (id,organization_id,plan_id,line_no,procedure_id,tooth_code,description,estimated_fee) values ('d1610000-0000-0000-0000-000000000001','b7200000-0000-0000-0000-000000000001','d1600000-0000-0000-0000-000000000002',1,'c9300000-0000-0000-0000-000000000001','26','Composite filling on 26.',2500.00)$$,'an item with an org-scoped procedure, valid FDI tooth, and bounded fee is accepted');
select extensions.ok((select line_no=1 and procedure_id='c9300000-0000-0000-0000-000000000001' and tooth_code='26' and description='Composite filling on 26.' and estimated_fee=2500.00 from public.treatment_plan_items where id='d1610000-0000-0000-0000-000000000001'),'the item persists its bounded projection');
select extensions.is((select estimated_fee_centavos from public.treatment_plan_items where id='d1610000-0000-0000-0000-000000000001'),250000::bigint,'a legacy decimal estimate derives the exact centavo value');
select extensions.throws_ok($$insert into public.treatment_plan_items (organization_id,plan_id,line_no,estimated_fee,description) values ('b7200000-0000-0000-0000-000000000001','d1600000-0000-0000-0000-000000000002',2,123.456,'fractional centavo fee')$$,'23514','treatment plan estimate cannot be represented exactly in centavos','a fractional-centavo estimate is rejected, never rounded');
select extensions.throws_ok($$insert into public.treatment_plan_items (organization_id,plan_id,line_no,estimated_fee,estimated_fee_centavos,description) values ('b7200000-0000-0000-0000-000000000001','d1600000-0000-0000-0000-000000000002',2,100.00,20000,'conflicting dual fee')$$,'23514','treatment plan estimate decimal and centavo values conflict','a conflicting dual estimate is rejected');
...
select extensions.is((select estimated_fee from public.treatment_plan_items where organization_id='b7200000-0000-0000-0000-000000000001' and plan_id='d1600000-0000-0000-0000-000000000002' and line_no=3),33.33::numeric,'a centavo-only estimate keeps the legacy decimal column readable');
...
select extensions.throws_ok($$insert into public.treatment_plan_items (organization_id,plan_id,line_no,estimated_fee,description) values ('b7200000-0000-0000-0000-000000000001','d1600000-0000-0000-0000-000000000002',2,-1,'negative fee')$$,'23514',null,'a negative estimated fee is rejected');
select extensions.throws_ok($$insert into public.treatment_plan_items (organization_id,plan_id,line_no,estimated_fee,description) values ('b7200000-0000-0000-0000-000000000001','d1600000-0000-0000-0000-000000000002',2,1000000000,'oversized fee')$$,'23514',null,'an estimated fee above 999999999 is rejected');
