...
insert into public.charges(id,organization_id,patient_id,branch_id,provider_id,procedure_id,amount_centavos,service_date,idempotency_key,created_by) values(${uuid(ids.charge)},${uuid(ids.organization)},${uuid(ids.patient)},${uuid(ids.branch)},${uuid(ids.provider)},${uuid(ids.procedure)},10000,current_date,${literal(`charge-${suffix}`)},${uuid(ids.user)}); commit;`;
  // Task 7: the visit-bound boundary is the one the composer calls. Two
  // concurrent submissions under the same derived request key must converge on
  // one canonical component graph AND one managed clinical visit.
  const request = `begin; set local role authenticated; select set_config('request.jwt.claim.role','authenticated',true); select set_config('request.jwt.claim.sub',${literal(ids.user)},true); select * from public.record_visit_implant_component_v2(${uuid(ids.branch)},${uuid(ids.patient)},'[{"tooth_fdi":"16","ordinal":1,"component_kind":"FIXTURE"}]'::jsonb,(timezone('Asia/Manila', statement_timestamp()))::date,${uuid(ids.charge)},null,${literal(key)}); commit;`;
  ...
  try {
    ...
    const proof = await execute(command, `select case when (select count(*) from public.dental_implant_components where organization_id=${uuid(ids.organization)})=1 and (select count(*) from private.odontogram_revamp_current_idempotency where organization_id=${uuid(ids.organization)} and idempotency_key=${literal(key)})=1 and (select count(*) from public.clinical_encounters where organization_id=${uuid(ids.organization)})=1 then 'IMPLANT_SAME_KEY_OK' else 'IMPLANT_SAME_KEY_FAILED' end;`, options); requireSuccess(proof, "implant concurrency proof"); if (!proof.stdout.includes("IMPLANT_SAME_KEY_OK")) throw new Error("concurrent same-key implant requests created more than one canonical graph or more than one managed visit");
