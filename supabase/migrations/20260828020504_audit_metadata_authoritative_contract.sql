-- Forward-only compatibility correction for the audit metadata validator.
--
-- Earlier feature migrations replaced this function from partial snapshots,
-- causing older migration-owned writers to fail after later migrations ran.
-- This is the authoritative union of the bounded keys emitted by the existing
-- workforce, scheduling, document, clinical, inventory, billing, and current
-- odontogram RPC writers. Unknown keys remain fail-closed.
  ...
  select case
    ...
    when exists (
      ...
      where not case
        ...
        when entry.key = 'note' then
          pg_catalog.jsonb_typeof(entry.value) = 'string'
          and pg_catalog.length(entry.value #>> '{}') <= 256
        when entry.key = 'idempotency_key' then
          pg_catalog.jsonb_typeof(entry.value) = 'string'
          and pg_catalog.length(entry.value #>> '{}') between 1 and 128
        when entry.key = 'cause' then
          pg_catalog.jsonb_typeof(entry.value) = 'string'
          and entry.value #>> '{}' in (
            'DIRECT_COST',
            'ATTRIBUTION',
...
comment on function private.audit_metadata_is_safe(jsonb) is
  'Authoritative fail-closed validator for the bounded non-sensitive metadata keys emitted by migration-owned audit writers through the current odontogram RPC boundary.';
