...
import { TreatmentPlanMode, type PlanAuthoringContext } from "./treatment-plan-mode";
...
function renderMode(overrides: Partial<Parameters<typeof TreatmentPlanMode>[0]> = {}, selectedFdi: readonly number[] = [26]) {
  const chart: (plan: PlanAuthoringContext | null) => React.ReactNode = vi.fn(() => <div data-testid="anatomical-chart" />);
  render(
...
describe("TreatmentPlanMode", () => {
  ...
  it("offers no lifecycle or authoring action once the plan is acknowledged", async () => {
    ...
    await waitFor(() =>
      expect(chart).toHaveBeenLastCalledWith(expect.objectContaining({ status: "ACKNOWLEDGED" })),
    );
  });
