...
const billing = vi.hoisted(() => ({ allocatePayment: vi.fn(), postCharge: vi.fn(), postChargeAdjustment: vi.fn(), recordPayment: vi.fn(), recordPostdatedCheque: vi.fn(), summarizeProcedureCharges: vi.fn() }));
vi.mock("next/cache", () => ({ revalidatePath: vi.fn() }));
vi.mock("@/lib/authorization", () => authorization);
vi.mock("@/lib/billing/service", () => billing);

import { allocatePaymentAction, postAdjustmentAction, postChargeAction, recordPaymentAction, summarizeProcedureChargesAction } from "./billing-actions";
