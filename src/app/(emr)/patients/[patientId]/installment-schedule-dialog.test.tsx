...
describe("InstallmentScheduleDialog", () => {
 it("labels rows as expectations and keeps actual ledger allocations separate", async () => {
  ...
  await user.type(screen.getByLabelText("Due date 1"), "09/01/2026"); await user.type(screen.getByLabelText("Expected centavos 1"), "50000"); await user.click(screen.getByRole("button", {name:/add installment/i})); await user.type(screen.getByLabelText("Due date 2"), "10/01/2026"); await user.type(screen.getByLabelText("Expected centavos 2"), "75000"); await user.click(screen.getByRole("button", {name:/review expectations/i})); expect(screen.getByRole("dialog").textContent).toMatch(/PHP 500.00/); await user.click(screen.getByRole("button", {name:/confirm and save/i}));
