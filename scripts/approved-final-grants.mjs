...
// The review-round-1 repair. This object migration is the file that REVOKES the
// browser grant on both superseded v3 relationship writers, so it — never a
// grants file — is the supersede pivot recorded on those two entries below.
const RELATIONSHIP_WORKFLOWS_V2_REPAIR_MIGRATION =
  "20260901010134_relationship_workflows_v2_repair.sql";
...
export const TERMINAL_MIGRATIONS = Object.freeze([
  ...
  Object.freeze({ file: ODONTOGRAM_REVAMP_IMPLANT_CHARGE_GRANTS_MIGRATION, grants: Object.freeze([{ grantee: "authenticated", objectClass: "function", object: "public.record_current_implant_component_v3(uuid,uuid,jsonb,timestamptz,uuid,text)", privilege: "execute", columns: [], supersededFrom: RELATIONSHIP_WORKFLOWS_V2_REPAIR_MIGRATION, supersededBy: "public.record_visit_implant_component_v2(uuid,uuid,jsonb,date,uuid,text,text)", reason: "Provider-free current implant writer retains a validated same-patient charge link while deriving provider server-side. Superseded by the visit-bound boundary and revoked from every browser role by 20260901010134: it opened no clinical visit, wrote a null encounter, and accepted an unbounded client-supplied occurrence time, so it bypassed both the managed-visit attribution and the one-year backdating window its replacement enforces." }]) }),
  Object.freeze({ file: ODONTOGRAM_REVAMP_DIRECT_CHARGE_AND_BRIDGE_GRANTS_MIGRATION, grants: Object.freeze([{ grantee: "authenticated", objectClass: "function", object: "public.record_current_bridge_v3(uuid,uuid,jsonb,timestamptz,uuid,text)", privilege: "execute", columns: [], supersededFrom: RELATIONSHIP_WORKFLOWS_V2_REPAIR_MIGRATION, supersededBy: "public.record_visit_bridge_v2(uuid,uuid,jsonb,date,uuid,text,text)", reason: "Provider-free current bridge writer retains a validated same-patient charge link while deriving provider server-side. Superseded by the visit-bound boundary and revoked from every browser role by 20260901010134: it opened no clinical visit, wrote a null encounter, and accepted an unbounded client-supplied occurrence time, so it bypassed both the managed-visit attribution and the one-year backdating window its replacement enforces." }]) }),
