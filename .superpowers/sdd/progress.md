...
Task 13: complete locally (commits 0e010e1, d18f90a, 89fb0e1, plus archive-processing hardening and the v3 DTO return-shape repair in 20260830010624; independent review finding fixed with guarded migration 20260830010623 and immutable archived-row trigger; clinical-photo pgTAP 33/33, odontogram RPC regression, focused gallery/action/workspace tests, full unit 1,563/1,563, typecheck, build, migration/security checks, and local forward migration verification pass. Cloud TEST, hosted E2E/axe, advisor/security, and final release gates remain pending; the pre-existing treatment_plans completion-marker residual remains documented).

# Controlled odontogram fork replacement ledger

Task 1: complete (commits c6aef0d..f57b27a, review approved after reset-control DOM regression coverage).
Task 2: complete (commits 59bd2d6..2826553, independent review approved after tooth-state, bridge-baseline, leakage/matrix, default-presence, and status-scoping fixes).
Task 3: complete (commits da725c7..2f4fbb5, independent review approved after touch reset removal, artifact parity, and reproducible source patch verification).
Task 4: complete (commits b562f55..d8ec95d, independent review approved after stable retry idempotency keys and route-identity cleanup).
Task 5: complete (commits 9defff9, 04b6694, plus the print projection/date/chronology follow-up; focused print suite 5/5 and targeted clinical suite pass; local build/typecheck/lint/security gates pass; Cloud TEST/hosted print verification pending).
Task 6: complete (commit 57e22fb plus final print/progress follow-up; controlled fork parity coverage, old renderer removal/unreference, E2E selector migration, notation switching, focused odontogram suite pass; Cloud TEST/hosted E2E/axe/release gates pending).
